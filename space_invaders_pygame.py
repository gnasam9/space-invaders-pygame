...
def collide(obj1,obj2):
	ofset_x=obj2.x-obj1.x
	ofset_y=obj2.y-obj1.y
	return obj1.mask.overlap(obj2.mask, (ofset_x,ofset_y))!=None
def main():
	run=True
	FPS=60
	...
	while run:
		...
		if keys[pygame.K_LEFT] and player.x-player_vel>0:
			player.x-=player_vel
		if keys[pygame.K_RIGHT] and player.x+player_vel+player.ge_width()<WIDTH:
			player.x+=player_vel
		if keys[pygame.K_UP] and player.y-player_vel>0:
			player.y-=player_vel
		if keys[pygame.K_DOWN] and player.y+player_vel+player.ge_height()+10<HEIGHT:
			player.y+=player_vel
		...
		for enemy in enemies[:]:
			...
			elif enemy.y+enemy.ge_height()>HEIGHT:
				lives-=1
				enemies.remove(enemy)
...
def main_menu():
	...
	title_font3=pygame.font.SysFont("comicsans",70)

	run=True
	while run:
		WIN.blit(BG,(0,0))
		title_label=title_font.render(" PRESS ANY KEY",1,(255,255,255))
		title_label2=title_font2.render("THE MOUSE TO BEGIN",1,(255,255,255))
		title_label3=title_font3.render("            OR",1,(255,255,255))
		WIN.blit(title_label,(WIDTH/2-title_label.get_width()/2,200))
		WIN.blit(title_label2,((WIDTH/2-title_label.get_width()/2)-50,400))
		WIN.blit(title_label3,((WIDTH/2-title_label.get_width()/2),300))
		pygame.display.update()
		for event in pygame.event.get():
			if event.type==pygame.QUIT:
				run=False
